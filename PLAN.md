...
|---|---------|------------------------|--------|
| П1 | Base64 (`fmt=v2b64`) и JSON (`fmt=xray-ext`) подписки **не импортируются** | Подписки начинаются со строк-комментариев `#profile-...`, парсер их не отфильтровывал | ✅ Исправлено (`Formats.kt`), CI зелёный |
| П0 | **main вообще не собирался** | `c56443e` ссылался на несуществующие drawable в `global_preferences.xml` | ✅ Исправлено (добавлены иконки), CI зелёный |
| П2 | Sing-box (`fmt=sing`) подписка импортируется, но **VPN не работает, пинга нет** | Outbound импортируется как `ConfigBean(TYPE_CONFIG)`; Auto-URL его исключал; рантайм требует логов | 🟡 Auto-URL теперь включает sing-box; рантайм-причину подтвердит тест на телефоне |
| П3 | UX/UI: лишние настройки, неудобный импорт (окно «выползает»), нет авто-группы и сортировки по пингу | Дизайн «как в оригинале» | 🔴 Сортировка по пингу и bottom-bar уже есть — нужна перекомпоновка |

**Ветка работы:** `auto-url-and-fixes` (CI зелёный). Релиз тегом `v1.4.2-ru.16` — в самом конце.
