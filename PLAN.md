...
**Ветка работы:** `auto-url-and-fixes` (CI зелёный).

### Релиз 1: `v1.4.2-ru.17` (тест ядра + новый UX настроек)
Сделано и CI-зелёное на ветке:
- ✅ П0 сборка, ✅ П1 импорт, ✅ удаление секретов, ✅ П2 auto-url для sing-box,
  ✅ импорт без диалога, ✅ упрощение настроек (расширенные скрыты), ✅ проверка
  безопасности (RU-защиты целы: локальный порт закрыт в VPN, bind=127.0.0.1, auth,
  HARDENING анти-инъекции).

Тег ставится на верхушку ветки (release.yml собирает подписанный APK из коммита тега);
`main` не трогаем до подтверждения тестом.

### Отложено на Релиз 2 (делать с визуальным фидбеком пользователя)
- 🔜 Нижняя навигация вместо боковой шторки (риск коллизии с StatsBar/FAB — нужен глаз).
- 🔜 Авто-группа для Auto-URL + явная сортировка по пингу (DB lifecycle — нужен тест).
- 🔜 Диагностика рантайма П2 по логам ядра с телефона.
