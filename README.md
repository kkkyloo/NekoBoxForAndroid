...
2. **Добавьте серверы/подписку:** Скопируйте ссылку на подписку и выберите в меню "Добавить профиль из буфера обмена".
3. **Включите Умный авто-выбор:** Зайдите в *Настройки -> Основные настройки* и включите галочку **Умный авто-выбор (Auto URL)**. Приложение автоматически выберет самый быстрый сервер и будет переключать вас, если текущий заблокируют.
4. **Настройте режим VPN:** *Настройки -> Режим VPN для приложений -> Включить -> Выбрать "Прокси"*. Отметьте только те приложения, которым нужен обход блокировок (рекомендуемый и самый безопасный режим).
