...
3. **Включите Умный авто-выбор:** Зайдите в *Настройки -> Основные настройки* и включите галочку **Умный авто-выбор (Auto URL)**. Приложение автоматически выберет самый быстрый сервер и будет переключать вас, если текущий заблокируют.
   * **Продвинутая настройка:** Там же, в разделе "Продвинутый Умный автовыбор", вы можете:
     * Изменить URL для пинга (по умолчанию Cloudflare).
     * Отфильтровать страны (например, исключить "ru, ua" или тестировать только "🇺🇸, 🇩🇪").
     * Исключить или оставить только конкретные подписки по их ID (если у вас несколько подписок).
